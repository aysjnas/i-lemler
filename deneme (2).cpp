...
Nokta::Nokta() {
    setX(0);//nokta sınıfını kullandık 00 orjin. 
    setY(0);
}

Nokta::Nokta(double value) { // tek parametreli tek değer girilen yapıcımız.
    setX(value);
    setY(value);
}

Nokta::Nokta(double x, double y) {// 2 parametreli yapıcımızı girdik.
    setX(x);
    setY(y);
}

Nokta::Nokta( const Nokta& otherNokta) {
    x=otherNokta.getX();
    y=otherNokta.getY();//kopya üreten yapıcım other diyen.
}
...
void Nokta::setX(double X) {// X in  privatedan publıc e set metotu .
    this->x = X;
}

double Nokta::getX()const {// x in get metodu oluşuyor private dan publıc e.
    return x;
}
// y in set metotu
...
void DogruParcasi::setP1(Nokta nokta1) {
    this->nokta1 = nokta1;
}
...
Nokta DogruParcasi::kesisimNoktasi(Nokta nokta) {
    ...
    Nokta noktadegiskeni{ x,y };
    return noktadegiskeni;
}

Nokta DogruParcasi::ortaNokta() {
    
    Nokta OrtaNokta{ (nokta1.getX() + nokta2.getX()) / 2, (nokta1.getY() + nokta2.getY()) / 2 };
    return OrtaNokta;
}
...
double* Ucgen::acilar() {
    ...
    double inner_product1 = acos(((uzunluk2 * uzunluk2) + (uzunluk3 * uzunluk3) - (uzunluk1 * uzunluk1))// iç açıları hesaplıyorum
    ...
    dizi[0] = inner_product1;
    dizi[1] = inner_product2;
    dizi[2] = inner_product3;
    return dizi;
}
